...
ROM = os.path.join(REPO, "roms", "1.3 IPDB latest", "V1 3_01.bin")
...
# dasmx86 from the repository root.
ROM_REL = "roms/1.3 IPDB latest/V1 3_01.bin"
DASM = os.path.expanduser("~/iomoon/tools/dasmxx/src/dasmx86")

ROM1_BASE = 0x80000          # flat address of ROM1 file offset 0
CODE_LO = 0xC0000            # UMCS window, start
CODE_HI = 0x100000           # UMCS window, end (exclusive)

ENTRIES = os.path.join(BASE, "entries.txt")
LABELS = os.path.join(BASE, "labels.txt")
CMD = os.path.join(BASE, "iomoon_80188.cmd")
LST = os.path.join(BASE, "iomoon_80188.lst")
...
def emit_cmd(entries, regions):
    lines = []
    lines.append("# IO Moon (SLEIC, 1994) -- 80188 game/sound ROM, V1.3 set chip 01")
    lines.append("# Generated by asm/baseline-2026-09/tools/grow.py -- do not hand-edit;")
    lines.append("# edit asm/baseline-2026-09/entries.txt and re-run instead.")
    lines.append("#")
    lines.append("# Listing addresses are FLAT 80188 addresses.  ROM1 is mapped at")
    lines.append("# 0x80000-0xFFFFF (file offset = linear - 0x80000); the 80188's UMCS")
    lines.append("# chip select only decodes 0xC0000-0xFFFFF, which is what is listed.")
    lines.append("#")
