...
**The `.lst` is parsed with the same line shape `grow.py` already uses**
(`grow.LINE_RE`, reused verbatim): four leading spaces, hex address, colon,
four spaces, space-separated hex byte list, mnemonic, operands. `DB`
byte-dump lines are skipped -- they are data, not instructions, and never
occur inside a `c`-opened region in this baseline (confirmed: dasmx86
disassembled every declared code region as code, 0 `???`/undecodable
bytes).
