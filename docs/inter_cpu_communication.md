...
| `4000:1158`+ | **Command queue body** — circular byte FIFO. Consumer **unresolved**: it is *not* the Z80 (no `4000h` access) and *not* the PIC at IC23 (DMD-only — see below); most likely an 80188 routine or DMA channel not yet located. See [`../research/80188_to_z80_mailbox.md`](../research/80188_to_z80_mailbox.md). |
| `4000:1200`+ | String buffer (text display) | 80188-internal |
| `4000:1220`+ | Secondary buffer | 80188-internal |

The PACS window at segment `4000h` is at least **32 KB wide** (`0x40000`–`0x47FFF`). A PinMAME debugger trace (see [`../research/pinmame_session_2/`](../research/pinmame_session_2/)) observed 80188 accesses up to physical `0x47B77`, well past the small `4000:11xx` command area documented here. The full extent of the window is determined by the PACS register value `0x41FC` written at boot and the 80188's internal chip-select decode.
...
The actual consumer is **unresolved**. It is not the Z80 (no access to segment `4000h`), and it is **not** the PIC 16C57 at IC23 either: every PIC pin on sheet `011-029-07` is a DMD signal (`RDATA`, `DE`, `RCLK`, `COLATCH`, `VA3`–`VA12`, `VSYNC`, `SCREEN`), so the PIC is purely a DMD raster controller with no path to the sound chips or to this queue. The most likely consumer is an 80188 routine or DMA channel not yet located (the 80188 owns both sound chips). Earlier drafts hypothesised PICs at IC34 and "IC57"; those turned out to be a 74LS glue chip and a PAL 20L10. See [`hardware_architecture.md`](hardware_architecture.md) for the chip inventory and [`../research/80188_to_z80_mailbox.md`](../research/80188_to_z80_mailbox.md) for the investigation. Resolving it means finding the consumer in the 80188 ROM (or confirming a DMA channel handles it) — not dumping IC23, which is the DMD PIC.
