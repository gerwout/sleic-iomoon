...
This wire-level protocol is **not what a PinMAME driver needs to model**. PinMAME's DMD core (`core_dmd_submit_frame`) takes a finished 128 × 32 grid of brightness values per visible frame. The driver's job stops *before* the wire — at the **80188-side** interface, which is:

- Frame buffer pointers at `4000:1150` (buffer 1) and `4000:1154` (buffer 2) in the 80188's work RAM (see [`inter_cpu_communication.md`](inter_cpu_communication.md)).
- DMD control registers at segment `A000h` (see [`dmd_graphics.md`](dmd_graphics.md)).

The PIC 16C54HS is essentially the rasterizer between those structures and the wire. It does **not** need to be emulated — the driver can decode the 2-bitplane frame buffer from the 80188's work RAM, merge the planes per the resolved plane-order convention, and hand the result to PinMAME's DMD core.

The wire protocol is useful for:

1. **Verifying the plane-order discrepancy** above by comparing a decoded ROM frame to the actual pixels captured in the .sal file.
2. **Measuring the true frame rate** (~72–75 fps visible), so PinMAME's VSYNC IRQ rate can be set accurately. Resolved: the measured panel rate is **~145 Hz** (see the table above) and is now applied in the driver — superseding the earlier stub's 120 Hz / 244 Hz placeholders.
3. **Validating the static-screen and animated-frame outputs** generated by `dmd_viewer.py` against ground truth from the panel.
