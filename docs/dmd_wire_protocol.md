...
- Frame buffer pointers at `4000:1150` (buffer 1) and `4000:1154` (buffer 2) in shared RAM (see [`inter_cpu_communication.md`](inter_cpu_communication.md)).
- DMD control registers at segment `A000h` (see [`dmd_graphics.md`](dmd_graphics.md)).

The PIC 16C54HS is essentially the rasterizer between those structures and the wire. It does **not** need to be emulated — the driver can decode the 2-bitplane frame buffer from shared RAM, merge the planes per the resolved plane-order convention, and hand the result to PinMAME's DMD core.
...
2. **Measuring the true frame rate** (~72–75 fps visible), so PinMAME's VSYNC IRQ rate can be set accurately. Current PinMAME stub uses 120 Hz or 244 Hz — both wrong.
