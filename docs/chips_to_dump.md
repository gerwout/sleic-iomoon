...
| IC8  | AMD PAL16L8A-2CN           | PDIP-20        | 011-030A (Z80)    | **Dumped** — archived at [`../roms/PAL16L8/`](../roms/PAL16L8/): raw sweeps, truth table, equations and JEDEC |
...
- **IC8** is a **bus-cycle decoder**. It supplies the I/O-read and I/O-write enables that gate the port decoders at IC16 and IC17, and the interrupt-acknowledge decode on the path that clears the Z80's `/INT` latch. It does no memory-map decoding: `A13`, `A12` and `A7` are wired to it and reach no output, and it generates no memory chip select and no memory read or write strobe — so IC5's and IC7's chip enables come from elsewhere on the board, and sheet `011-030-01` does not describe what is on pins 12, 16, 18 and 19. Full reading: [`../roms/PAL16L8/README.md`](../roms/PAL16L8/README.md).
...
## IC8 — PAL 16L8A-2CN  (Z80 board, 011-030A)  ✅ DUMPED

The chip is locked, so Path B below is the route that produced the dump: the same dupico (DuPAL V3) rig that recovered IC7, sweeping all 2048 combinations of eleven inputs and reading six outputs. The JEDEC reproduces the measurement on every one of them, the whole pipeline has been run twice independently to byte-identical artefacts, and pin direction is established in four configurations. The device is a bus-cycle decoder and the memory-map decode is not in it — which is a property of the part, not a gap in the read. What is still open is a board-wiring question: which device drives IC5's and IC7's chip enables. Dump, equations and evidence: [`../roms/PAL16L8/`](../roms/PAL16L8/).
...
- **What the dump leaves open**: the part read is the device desoldered from this machine's own board, and it generates **no memory chip select and no memory read or write strobe** — pin 12 never drives, pin 16 reduces to `/M1`, `A13` appears in no product term, and every output that responds to `/PRD` or `/PWR` also requires `/IOREQ`. No relabelling of the six outputs supplies them, so the PCB does not carry the nets sheet 011-030-01 puts on those pins, and IC5's and IC7's chip enables come from somewhere else. Two outputs also sit on each other's labels. [`../roms/PAL16L8/README.md`](../roms/PAL16L8/README.md) carries the evidence and the board measurements that would close it.
...
- **IC8** ✅ **(dumped)** — the two port-decoder enables and the interrupt-acknowledge decode, which together confirm that the Z80's five input ports and eight output ports are the whole of its I/O, and the `/RI` pulse that clears the interrupt latch. As a negative result, and a structural one: three of the five address lines wired to the part reach no output, so it does no memory-map decoding and IC5's and IC7's chip enables must come from elsewhere on the board.
