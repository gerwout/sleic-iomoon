...
same year as IO Moon. Only the **Z80 I/O CPU ROM** is dumped. It was read off the
Z80 board's 27C256 EPROM (an ST27C256, hence the filename). No public dump of this
...
|----------|------|--------------|-----|---------|
| `ST27C256-z80.bin` | 32,768 bytes (32 KB) | `358bd508dd8232bbfe9d8d14465015d3` | Z80 | I/O CPU ROM — 18,678 bytes used, remainder `0xFF` |

## Missing

Everything except the Z80 ROM. By analogy with IO Moon and Bike Race, a complete
set would additionally need the 80188 game/graphics ROMs, the OKI MSM6376 sample
ROM(s), and the display coprocessor ROM. Without those the machine cannot be
emulated; this image alone documents the I/O layer.
...
So Doña Elvira 2's I/O firmware is a sibling of the IO Moon / Bike Race Z80 code
and shares essentially nothing with the older Sleic Pin-Ball Z80 code. Its overall
size profile agrees: 18,678 bytes used, against 18,635 for Bike Race and 20,437 for
IO Moon, versus 8,763 for Sleic Pin-Ball.
