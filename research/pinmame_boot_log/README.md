...
runtime: 15s wall, ~1800 emulated 80188 IRQ ticks at 120 Hz
...
2. Added shared-RAM mapping at `0x40000-0x41FFF` so the 80188 can read/write segment `4000h` (PACS-mapped) without filling the log with "unmapped" warnings.
...
3. **No undocumented A000h offset was written** in the ~15 s of emulated runtime. Combined with the static analysis already showing that the 80188 has only two `OUT` instructions (boot config) and no MPCS chip-select, this is **strong empirical evidence for hypothesis (2) in [`ym3812_pinmame_precedents.md`](../../docs/ym3812_pinmame_precedents.md)** — the YM3812 is physically populated on the IO Moon CPU board for BOM compatibility with the older SLEIC machines, but **never accessed by the IO Moon software**.
4. **Stub blocker identified.** The 80188 hangs at `PC=0x40024` (segment `4000h`, in shared RAM) after about 180 IRQ ticks. The IRQ-tick PCs trace a clean boot path through the `F000h` system code, into `0x01E9C`/`0x03D40`/`0x05BE4` (early game-state setup in ROM2 graphics region), then `0x711AF`/`0x642F3` (graphics-table lookups), then a runaway loop at `0x40024` (executing what is supposed to be code from a location that, in real hardware, is loaded into LMCS-mapped ROM2 graphics but in the current PinMAME stub is unmapped).
5. **Caveat on (3)**: 1800 IRQ ticks is "boot only". The 80188 never reached attract-mode-running state, so if the YM3812 *is* used by IO Moon at all, it would only be during game state we have not yet exercised. To make the conclusion definitive, the next debug session needs ROM2 mapped into REGION_CPU1 at `0x00000-0x7FFFF` (overlaying the shared-RAM window at `0x40000-0x41FFF`).
...
1. Define a new `SLEIC_ROMSTART_IOMOON` (or modify `SLEIC_ROMSTART5` only for IO Moon) so ROM2 (v1_3_02.bin) is loaded into `REGION_CPU1` at `0x00000-0x7FFFF` instead of into `REGION_USER2`. The shared-RAM PACS window at `0x40000-0x41FFF` will overlay ROM2 — that matches real hardware (PACS chip select has priority for that small window).
...
3. Look for any A000h write to an offset *other than* 0x000/0x080/0x200/0x300. If a full attract → game loop produces zero such writes, hypothesis (2) is conclusively confirmed and the YM3812 can stay declared-but-unmapped in PinMAME indefinitely. If a new offset appears, it's the YM3812 base.
